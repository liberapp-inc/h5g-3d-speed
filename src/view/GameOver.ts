...
class GameOver extends GameObject{
    ...
    constructor() {
        ...
        GameOver.I = this;
        this.texts[0] = Util.newTextField("SCORE : " + Score.I.point.toFixed(), Util.width / 12, PLAYER_COLOR, 0.5, 0.35, true, false);
    ...
    update() {
        ...
        if( this.step == this.fadeInFrame ){
            this.retryButton = new Button("リトライ", Util.width/16, BACK_COLOR, 0.50, 0.65, 0.4, 0.1, PLAYER_COLOR, 1.0, this.onTapRetry );
            
            if( Score.I.point > Score.I.bestScore ){
                egret.localStorage.setItem(SAVE_KEY_BESTSCORE, Score.I.point.toFixed() ); // string
                this.texts[1] = Util.newTextField("NEW RECORD!", Util.width / 13, PLAYER_COLOR, 0.5, 0.45, true, false);
